...
mosCurrents = zeros(size(MOSName,2),sweepTimes);
diodeCurrents = zeros(size(Diodes,2),sweepTimes);
for i = 1 : sweepTimes
    %修改作扫描的值
    ...
    x_res(:,i) = [0; DCres('x')];
    mosCurrents(:,i) = DCres('MOS')';
    diodeCurrents(:,i) = DCres('Diode')';
end
Values((1 : nvNum),:) = x_res(plotnv,:);
...
if(~isempty(VIndexInValues))
    Values(VIndexInValues,:) = Values(VIndexInValues) .* x_res(VIndexInDCres,:);
end
%更改Values结果里要的电源电流I
if(~isempty(IIndexInValues))
    Values(IIndexInValues,:) = Values(IIndexInValues) .* Value(IIndexInValue,:);
end
%更改Values结果里要的电阻电流
if(~isempty(RIndexInValues))
    Values(RIndexInValues,:) = Values(RIndexInValues) .* (x_res(RNodeIndexInDCresN1,:) - x_res(RNodeIndexInDCresN2,:));
end
end

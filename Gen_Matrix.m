...
%             A(pNum2,pNum2)= A(pNum2,pNum2)+1/cpValue;
        ...
        case 'V'
            ...
            x{nodeNums+INum}=['I_' CellName];
%             A(nodeNums+INum,nodeNums+INum)= 0;
            A = deleteElement(A,nodeNums+INum,nodeNums+INum);
            b(nodeNums+INum)=0;
            % 电压源贴入矩阵，影响两个节点电压，
            % 引入的电流影响两个节点电流
%             A(pNum1,nodeNums+INum)= A(pNum1,nodeNums+INum) + 1;
...
%             A(nodeNums+INum,pNum1)= A(nodeNums+INum,pNum1) + 1;
%             A(nodeNums+INum,pNum2)= A(nodeNums+INum,pNum2) - 1;
            A = renewElement(A,pNum1,nodeNums+INum,1);
            A = renewElement(A,pNum2,nodeNums+INum,-1);
            A = renewElement(A,nodeNums+INum,pNum1,1);
            A = renewElement(A,nodeNums+INum,pNum2,-1);
            % 更新对应的b向量，即一个电压方程
            b(nodeNums+INum)=b(nodeNums+INum)+cpValue;
        case{'G','E'}
            % 压控电压源 (VCVS) - E
...
%                 A(nodeNums+INum,cpNum1)= A(nodeNums+INum,cpNum1) - cpValue;
%                 A(nodeNums+INum,cpNum2)= A(nodeNums+INum,cpNum2) + cpValue;
                A = renewElement(A,nodeNums+INum,cpNum1,-cpValue);
                A = renewElement(A,nodeNums+INum,cpNum2,cpValue);
            end
...
%                 A(pNum1,nodeNums+INum)= A(pNum1,nodeNums+INum) + cpValue;
%                 A(pNum2,nodeNums+INum)= A(pNum2,nodeNums+INum) - cpValue;
                A = renewElement(A,pNum1,nodeNums+INum,cpValue);
                A = renewElement(A,pNum2,nodeNums+INum,-cpValue);
                
                % 这里缺失一个方程
                % 根据这里的器件是什么决定如何增添方程
%                 A(nodeNums+INum,nodeNums+INum) = A(nodeNums+INum,nodeNums+INum) + 1;
                A = renewElement(A,nodeNums+INum,nodeNums+INum,1);
...
%                         A(nodeNums+INum,N1(cpIndex))=A(nodeNums+INum,N1(cpIndex)) - 1/Value(cpIndex);
%                         A(nodeNums+INum,N2(cpIndex))=A(nodeNums+INum,N2(cpIndex)) + 1/Value(cpIndex);
                        A = renewElement(A,nodeNums+INum,N1(cpIndex),-1/Value(cpIndex));
                        A = renewElement(A,nodeNums+INum,N2(cpIndex),1/Value(cpIndex));
                        % 电导与电流的关系
                    case {'V','E','H'}
                        % 如果为电压源，这里应该去寻找电压源对应的电流
                        % 如果是其他源的电流，很可能在这里还没有生成，所以还需要另外的步骤去寻找这样一个电流
                        ...
                        ICdName_Index=find(strcmp(x, ['I_' CdName]));
%                         A(nodeNums+INum, ICdName_Index) = A(nodeNums+INum, ICdName_Index) - 1;
                        A = renewElement(A,nodeNums+INum,ICdName_Index,-1);
                    case {'I','G','F'}
                        % 如果为电流源、受控电流源，则会有一个电流相等的关系
                        ICdName_Index=find(strcmp(x, ['I_' CdName]));
                        if CdName(1) == 'I'
%                             A(nodeNums+INum, ICdName_Index) = A(nodeNums+INum, ICdName_Index) - 1;
                        ...
                        elseif CdName(1) == 'F'
                            ...
                            IcontrolCS_Index=find(strcmp(x,['Icontrol_' CdName{1}]));
%                             A(nodeNums+INum, IcontrolCS_Index) = A(nodeNums+INum, IcontrolCS_Index) - Value(cpIndex);
                            A = renewElement(A,nodeNums+INum,IcontrolCS_Index,-Value(cpIndex));
                        elseif CdName(1) == 'G'
%                             A(pNum1,cpNum1)= A(pNum1,cpNum1) - cpValue;
%                             A(pNum1,cpNum2)= A(pNum1,cpNum2) + cpValue;
                            A = renewElement(A,pNum1,cpNum1,-cpValue);
...
%                 A(m,pNum2)= A(m,pNum2) - 1;
%                 A(m,k) = A(m,k)-cpValue;
                A = renewElement(A,pNum1,m,1);
                A = renewElement(A,pNum2,m,-1);
                A = renewElement(A,m,pNum1,1);
                A = renewElement(A,m,pNum2,-1);
                A = renewElement(A,m,k,-cpValue);

                % 这里缺失一个方程
%                 A(k,k) = A(k,k) + 1;
                A = renewElement(A,k,k,1);
                % 第k行是新的等式
                switch CdName(1)
                    case 'R'
                        % 如果为电阻则
%                         A(k,N1(cpIndex))=A(k,N1(cpIndex)) - 1/Value(cpIndex);
%                         A(k,N2(cpIndex))=A(k,N2(cpIndex)) + 1/Value(cpIndex);
                        A = renewElement(A,k,N1(cpIndex),-1/Value(cpIndex));
                        A = renewElement(A,k,N2(cpIndex),1/Value(cpIndex));
                        % 电导与电流的关系
                    case {'V','E','H'}
                        % 如果为电压源，这个时候这条支路上一定会引入一个电流
                        % 如果是其他源的电流，很可能在这里还没有生成，所以还需要另外的步骤去寻找这样一个电流
                        ...
                        ICdName_Index = find(strcmp(x, ['I_' CdName]));
%                         A(k, ICdName_Index) = A(k, ICdName_Index) - 1;
                        A = renewElement(A,k,ICdName_Index,-1);
                    case {'I','G','F'}
                        % 如果为电流源、受控电流源，则会有一个电流相等的关系
                        ICdName_Index = find(strcmp(x, ['I_' CdName]));
                        if CdName(1) == 'I'
%                             A(k, ICdName_Index) = A(k, ICdName_Index) - 1;
                        ...
                        elseif CdName(1) == 'F'
                            ...
                            IcontrolCS_Index = find(strcmp(x,['Icontrol_' CdName]));
%                             A(k, IcontrolCS_Index) = A(k, IcontrolCS_Index) - Value(cpIndex);
                            A = renewElement(A,k,IcontrolCS_Index,-Value(cpIndex));
                        elseif CdName(1) == 'G'
%                             A(pNum1,cpNum1)= A(pNum1,cpNum1) - cpValue;
%                             A(pNum1,cpNum2)= A(pNum1,cpNum2) + cpValue;
                            A = renewElement(A,pNum1,cpNum1,-cpValue);

...
while ~feof(fid)
    ...
    elseif ~isempty(tokens_BJTModel)
        % 匹配的正则表达式
        % ########################## BJTModel的格式如下 ############################
        % ########### .BIPOLAR 1 Js 1e-16 alpha_f 0.995 alpha_r 0.05 Cje 1e-11 Cjc 1e-12 #############
        % ################ Js 1e-16 的单位是A/um^2, 而BJT定义中的Junctionarea单位是um^2 #################
        expr = ['\.BIPOLAR\s+(\d+)\s+Js\s+([\+\-]?\d*\.?\d+(?:[eE][\+\-]?\d+)?)' ...
            '\s+alpha_f\s+([\+\-]?\d*\.?\d+(?:[eE][\+\-]?\d+)?)\s+' ...
            'alpha_r\s+([\+\-]?\d*\.?\d+(?:[eE][\+\-]?\d+)?)\s+'...
            'Cje\s+([\+\-]?\d*\.?\d+(?:[eE][\+\-]?\d+)?)\s+'...
            'Cjc\s+([\+\-]?\d*\.?\d+(?:[eE][\+\-]?\d+)?)'
            ];
        % 在这里已经匹配到了模型数据
...
% 提取MOS、BJT的寄生电容，其中BJT的寄生电容值在Generate_ACnetlist.m中应该能根据Vbe、Vbc的值更新
